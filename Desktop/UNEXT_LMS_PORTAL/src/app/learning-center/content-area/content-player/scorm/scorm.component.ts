...
export class ScormComponent {
  ...
  ngOnInit(): void {
    ...
    this.elementData = this.storageService.get(StorageKey.ELEMENT_DETAIL);
  ...
  ngAfterViewInit(): void {
    ...
    this.iframeHolder.nativeElement.src = `/api/courseservice/content-store/getHTML/index_lms_html5.html`;
    // this.iframeHolder.nativeElement.src = `${baseUrl}/api/courseservice/content-store/getHtml/index.html`;
  }
  ...
  storeProgressTimeLastAccessed(): void {
    const scormProgressDetails = this.apiService1_2.scormProgressDetails();
    const timeDifference = new Date().getTime() - this.timespent.getTime();
    const payload = {
      progress: scormProgressDetails.progress,
      lastAccessedPoint: scormProgressDetails.lastAccessedPoint,
